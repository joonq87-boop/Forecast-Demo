...
st.set_page_config(page_title="Revenue Optimizer — ForecastIQ Demo", page_icon="📊", layout="wide")
...
with tabs[2]:
        ...
        with cr2:
            ...
            if not demand_sigs:
                demand_sigs = [
                    ...
                    {"signal":"Indonesia implementing new halal certification requirements effective July 2026 — compliance documentation affecting cross-border F&B shipments","source":"BPJPH Advisory / ASEAN Food Regulatory Monitor 2026","impact":"Potential 2-3 week fulfilment delays for Indonesia-bound SKUs during transition","forecast_adjustment":"Build 2-week safety stock buffer for ID-destined SKUs; front-load June shipments"},
                ]
    ...
    if st.session_state.done:
        ...
        if not supply_risks:
            ...
            supply_risks = [
                {"risk":"Port congestion at PSA Singapore — container dwell times increased 18% in Q1 2026 due to Red Sea rerouting backlog","source":"PSA Terminal Operations Report Q1 2026 / Singapore Shipping Association","severity":"Medium","mitigation":"Pre-book slots 48hrs ahead; diversify to Tanjung Pelepas for non-urgent shipments; build 3-day buffer into fulfilment SLAs"},
                {"risk":"Raw material price volatility — palm oil futures up 14% YTD, packaging resin +9%. Margin compression accelerating across SEA F&B","source":"Bursa Malaysia Palm Oil Index / ICIS Packaging May 2026","severity":"High","mitigation":"Lock 90-day forward contracts on key inputs; activate pricing pass-through clauses with top 5 accounts; review SKU profitability monthly"},
                {"risk":"Labour shortage in warehouse operations — Singapore manufacturing vacancy rate at 4.2%, highest since 2022","source":"MOM Labour Market Report Q1 2026 / ASEAN Manufacturing PMI","severity":"Medium","mitigation":"Accelerate WMS automation for pick/pack; cross-train existing staff; evaluate 3PL partnership for peak periods"},
                {"risk":"Indonesia halal certification overhaul effective July 2026 — new documentation requirements for cross-border F&B shipments","source":"BPJPH Regulatory Advisory / ASEAN Food Safety Council 2026","severity":"Low","mitigation":"Audit current product labels against new BPJPH requirements; build 60-day compliance buffer; brief logistics partners on documentation changes"},
            ]
